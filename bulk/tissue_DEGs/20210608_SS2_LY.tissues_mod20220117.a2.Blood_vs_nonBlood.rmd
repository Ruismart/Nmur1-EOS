...
Aidx = grep("Blood",colnames(mat_pc))
Bidx = grep("Blood",colnames(mat_pc),invert = T)
...
p1 = p1 + ggrepel::geom_text_repel(mapping = aes(label=pca_d$batch),size=3.5, show.legend = F)
...
```{r eval=FALSE, include=FALSE}
write.table(rec_gcol(data.frame(ret$DEG_edgeR)), paste0("edgeR_DEGs.tissues_a2_",
