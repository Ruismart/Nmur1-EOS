...
Aidx = grep("SP",colnames(mat_pc))
Bidx = grep("SP",colnames(mat_pc),invert = T)
...
genes <- c(rets$up[1:13],rets$down[1:40])
pheatmap::pheatmap(log2(mat_pc[genes,c(Aidx,Bidx)]+1),cluster_rows = F, cluster_cols = F,
         fontsize_row = 8,
         main = "top40 log2(CPM+1)", gaps_row = c(13), gaps_col = length(Aidx))
```
...
pheatmap::pheatmap(log2(mat_pc[genes,c(Aidx,Bidx)]+1),cluster_rows = F, cluster_cols = F,
         ...
         scale = 'row',
         main = "top40 zscore", gaps_row = c(13), gaps_col = length(Aidx))
...
```{r eval=FALSE, include=FALSE}
write.table(rec_gcol(data.frame(ret$DEG_edgeR)), paste0("edgeR_DEGs.tissues_a3_",
